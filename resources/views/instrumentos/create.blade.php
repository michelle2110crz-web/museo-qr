@extends('layouts.app')

@section('title', 'Nuevo Instrumento')

@section('content')
<div class="container mx-auto px-4 py-8 max-w-2xl">
    <div class="card-glass p-6 md:p-8">
        <h1 class="font-display text-3xl text-[#c49a6c] text-center">🎵 Nuevo Instrumento</h1>
        <hr class="border-[#2a2a2a] my-6">

        <form action="{{ route('instrumentos.store') }}" method="POST" enctype="multipart/form-data">
            @csrf

            <div class="mb-4">
                <label class="block text-[#c49a6c] font-semibold text-sm">Nombre *</label>
                <input type="text" name="nombre" class="input-dark" required>
            </div>

            <div class="mb-4">
                <label class="block text-[#c49a6c] font-semibold text-sm">Nombre Original</label>
                <input type="text" name="nombre_original" class="input-dark">
            </div>

            <div class="mb-4">
                <label class="block text-[#c49a6c] font-semibold text-sm">Familia *</label>
                <input type="text" name="familia" class="input-dark" required>
            </div>

            <div class="mb-4">
                <label class="block text-[#c49a6c] font-semibold text-sm">Origen *</label>
                <input type="text" name="origen" class="input-dark" required>
            </div>

            <div class="mb-4">
                <label class="block text-[#c49a6c] font-semibold text-sm">Historia *</label>
                <textarea name="historia" rows="3" class="input-dark" required></textarea>
            </div>

            <div class="mb-4">
                <label class="block text-[#c49a6c] font-semibold text-sm">Características</label>
                <textarea name="caracteristicas" rows="2" class="input-dark"></textarea>
            </div>

            <div class="mb-4">
                <label class="block text-[#c49a6c] font-semibold text-sm">Uso Cultural</label>
                <textarea name="uso_cultural" rows="2" class="input-dark"></textarea>
            </div>

            <div class="mb-4">
                <label class="block text-[#c49a6c] font-semibold text-sm">📷 Imagen</label>
                <input type="file" name="imagen" class="input-dark file:bg-[#c49a6c] file:text-[#0a0a0f] file:font-bold file:border-0 file:rounded-lg file:px-4 file:py-2">
            </div>

            <div class="mb-4">
                <label class="block text-[#c49a6c] font-semibold text-sm">🎬 Video</label>
                <input type="file" name="video" class="input-dark file:bg-[#c49a6c] file:text-[#0a0a0f] file:font-bold file:border-0 file:rounded-lg file:px-4 file:py-2">
            </div>

            <div class="mb-4">
                <label class="block text-[#c49a6c] font-semibold text-sm">🎧 Audio</label>
                <input type="file" name="audio" class="input-dark file:bg-[#c49a6c] file:text-[#0a0a0f] file:font-bold file:border-0 file:rounded-lg file:px-4 file:py-2">
            </div>

            <div class="mb-6 flex items-center gap-3">
                <input type="checkbox" name="tiene_visita_virtual" value="1" id="virtual" class="accent-[#c49a6c] w-5 h-5">
                <label for="virtual" class="text-[#c49a6c] font-semibold text-sm">📱 Tiene visita virtual</label>
            </div>

            <div class="flex flex-wrap gap-4">
                <button type="submit" class="btn-glow inline-block">💾 Guardar</button>
                <a href="{{ route('instrumentos.index') }}" class="btn-outline-glow inline-block">← Volver</a>
            </div>
        </form>
    </div>
</div>

<style>
    .btn-glow {
        background: linear-gradient(135deg, #c49a6c, #a87d5a);
        color: #0a0a0f;
        font-weight: 700;
        padding: 12px 32px;
        border-radius: 50px;
        transition: all 0.4s ease;
        text-decoration: none;
        display: inline-block;
        border: none;
        cursor: pointer;
    }
    .btn-glow:hover {
        transform: translateY(-2px);
        box-shadow: 0 0 40px rgba(196,154,108,0.2);
        background: linear-gradient(135deg, #d4aa7c, #b88d6a);
    }
    .btn-outline-glow {
        background: transparent;
        color: #c49a6c;
        font-weight: 600;
        padding: 12px 32px;
        border-radius: 50px;
        border: 1px solid rgba(196,154,108,0.25);
        transition: all 0.4s ease;
        text-decoration: none;
        display: inline-block;
    }
    .btn-outline-glow:hover {
        background: rgba(196,154,108,0.08);
        border-color: #c49a6c;
        box-shadow: 0 0 40px rgba(196,154,108,0.08);
        transform: translateY(-2px);
        color: #c49a6c;
    }
</style>
@endsection